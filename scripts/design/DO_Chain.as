package design
{
   ...
   public class DO_Chain extends DesignObject
   {
      ...
      public function DO_Chain(parent:DisplayObjectContainer)
      {
         super(parent,GameCamera.CHAIN);
         this._data = new DO_ChainData();
      }
      ...
      override public function drawProperties() : void
      {
         ...
         this._heightSlider.minimum = 50;
         this._heightSlider.maximum = 250;
      ...
      {
         var height:Number = int(this._heightSlider.value / 10) * 10;
      ...
      override protected function drawUpdate() : void
      {
         ...
         this._drawBody.graphics.lineStyle(1.5,65280,1);
         this._drawBody.x = this._data.x;
         this._drawBody.y = this._data.y;
         var i:int = 0;
         var count:int = this._data.height / this._data.RADIUS / 2;
         for(i = 0; i < count; i++)
         {
            this._drawBody.graphics.drawCircle(0,(i + 0.5) * 2 * this._data.RADIUS * this._data.scale,this._data.RADIUS);
            this._drawBody.graphics.drawCircle(0,(i + 0.5) * 2 * this._data.RADIUS * this._data.scale,this._data.RADIUS / 2);
         }
      }
      ...
      override public function drawRealCostume() : void
      {
         ...
         _realCostume.graphics.beginFill(11184810,1);
         _realCostume.graphics.lineStyle(1,0,1);
         var i:int = 0;
         var count:int = this._data.height / this._data.RADIUS / 2;
         for(i = 0; i < count; i++)
         {
            _realCostume.graphics.drawCircle(0,(i + 0.5) * 2 * this._data.RADIUS * this._data.scale,this._data.RADIUS);
            _realCostume.graphics.drawCircle(0,(i + 0.5) * 2 * this._data.RADIUS * this._data.scale,this._data.RADIUS / 2);
         }
      ...
      {
         var xpos:Number = properX(Math.round(_parent.mouseX / G_INTERVAL) * G_INTERVAL);
         var ypos:Number = properY(Math.round(_parent.mouseY / G_INTERVAL) * G_INTERVAL);
         this._data.x = xpos;
         this._data.y = ypos;
         this.drawUpdate();
      }
      ...
      {
         this._data = newData as DO_ChainData;
         this.drawRealCostume();
      }
   }
}
