package effects
{
   ...
   public class ExplosionEffect extends UnitEffect
   {
       
      
      public function ExplosionEffect(x:Number, y:Number, effect:Number, duration:int)
      {
      ...
      override public function update() : void
      {
         ...
         for(var i:int = _list.length - 1; i >= 0; i--)
         {
            ...
            bubble.curY += bubble.current * bubble.speed * Math.cos(bubble.angle);
            bubble.curX += bubble.current * bubble.speed * Math.sin(bubble.angle);
