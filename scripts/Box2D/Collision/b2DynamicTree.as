   ...
   public class b2DynamicTree
   {
      ...
      public function Rebalance(iterations:int) : void
      {
         ...
         for(var i:int = 0; i < iterations; i++)
         {
            ...
            {
               node = Boolean(this.m_path >> bit & 1) ? node.child2 : node.child1;
               bit = bit + 1 & 31;
            }
            ++this.m_path;
      ...
      public function Query(callback:Function, aabb:b2AABB) : void
      {
         ...
         while(count > 0)
         {
            ...
            if(node.aabb.TestOverlap(aabb))
            {
               ...
               {
                  var _loc8_:* = count++;
                  stack[_loc8_] = node.child1;
                  var _loc9_:* = count++;
                  stack[_loc9_] = node.child2;
               }
            }
         }
      }
         ...
         while(count > 0)
         {
            ...
            if(node.aabb.TestOverlap(segmentAABB) != false)
            {
               ...
               if(separation <= 0)
               {
                  ...
                  {
                     var _loc20_:* = count++;
                     stack[_loc20_] = node.child1;
                     var _loc21_:* = count++;
                     stack[_loc21_] = node.child2;
                  }
               }
            }
         }
