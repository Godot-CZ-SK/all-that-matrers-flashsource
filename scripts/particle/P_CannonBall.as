package particle
{
   ...
   public class P_CannonBall extends Particle
   {
      
      public static const MAIN:String = "cannon_ball_main";
       
      
      protected var _data:DO_CannonBallData;
      
      protected var _master:P_Cannon;
      
      public function P_CannonBall(layerNo:int, data:DO_CannonBallData, master:P_Cannon)
      {
         this._data = data;
         this._master = master;
         super(layerNo);
      ...
      override public function create() : void
      {
         ...
         fd.maskBits = ~8;
         f1.SetUserData(MAIN);
         var speed:b2Vec2 = new b2Vec2(3.5 * Math.cos(this._data.angle),3.5 * Math.sin(this._data.angle));
      ...
      private function queryCallback(fix:b2Fixture) : Boolean
      {
         ...
         if(p is P_CannonBall || p == this._master || p is P_Chain || fix.IsSensor())
         {
            return true;
         }
         this.die();
      ...
      {
         if(p == this._master || p is P_Chain)
         {
            return;
         }
         this.die();
