package particle
{
   ...
   public class P_BonusSeeker extends Particle
   {
      
      public static const MAIN:String = "bonus_seeker_main";
      ...
      public function P_BonusSeeker(layerNo:int, data:DO_SeekerData)
      {
         this._data = data;
         super(layerNo);
      }
      ...
      private function queryCallback(fix:b2Fixture) : Boolean
      {
         ...
         if(p is P_BonusSpawnDoor || p is P_BonusSeeker || p is P_Unit || fix.IsSensor())
         {
            return true;
         }
         this.explode();
      ...
      private function trackUnit() : void
      {
         ...
         TweenLite.to(_costume,1,{"rotation":desiredAngle});
      ...
      private function searchForUnit() : void
      {
         ...
         if(this._search >= 30)
         {
            ...
            distances = [];
            for(i = 0; i < GM.ins.bl.units.length; i++)
            {
               unit = GM.ins.bl.units[i];
